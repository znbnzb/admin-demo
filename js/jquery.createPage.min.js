...
(function(b) {
        ...
        fillHtml: function(d, c) {
            return (function() {
                ...
                if (c.current > 1) {
                    ...
                    d.remove('.prevPage')
                    d.append('<span class="disabled prevPage">上一页</span>')
                }
                if (c.current != 1 && c.current >= 4 && c.pageCount != 4) {
                    d.append('<a href="javascript:;" class="tcdNumber">' + 1 + '</a>')
                }
                ...
                for (; f <= e; f++) {
                    if (f <= c.pageCount && f >= 1) {
                        if (f != c.current) {
                            d.append('<a href="javascript:;" class="tcdNumber">' + f + '</a>')
                        } else {
                ...
                if (
                    c.current != c.pageCount &&
                    c.current < c.pageCount - 2 &&
                    c.pageCount != 4
                ) {
                    d.append(
                        '<a href="javascript:;" class="tcdNumber">' + c.pageCount + '</a>'
                    )
                }
                if (c.current < c.pageCount) {
                    ...
                    d.remove('.nextPage')
                    d.append('<span class="disabled nextPage">下一页</span>')
                }
            })()
        },
        bindEvent: function(d, c) {
            return (function() {
                d.on('click', 'a.tcdNumber', function() {
                    var e = parseInt(b(this).text())
                    a.fillHtml(d, { current: e, pageCount: c.pageCount })
                ...
                d.on('click', 'a.prevPage', function() {
                    ...
                    a.fillHtml(d, { current: e - 1, pageCount: c.pageCount })
                    if (typeof c.backFn == 'function') {
                        c.backFn(e - 1)
                    }
                })
    ...
    b.fn.createPage = function(d) {
        var c = b.extend({ pageCount: 10, current: 1, backFn: function() {} }, d)
        a.init(this, c)
    }
})(jQuery)